# AGENTS.md - AI Agent Development Guide for tap-trentinopvgis
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from TrentinoPvGis.

### Key Components

1. **Tap Class** (`tap_trentinopvgis/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_trentinopvgis/client.py`): Handles API communication and authentication
1. **Streams** (`tap_trentinopvgis/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_trentinopvgis/streams.py`
...
```python
class MyNewStream(TrentinoPvGisStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(TrentinoPvGisStream):
...
```bash
tap-trentinopvgis --config config.json --discover
tap-trentinopvgis --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_trentinopvgis/tap.py`
...
```bash
# .env.example
TAP_TRENTINOPVGIS_API_URL=https://api.example.com
TAP_TRENTINOPVGIS_API_KEY=your_api_key_here
TAP_TRENTINOPVGIS_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-trentinopvgis --version`), Python version, and a minimal reproduction case when reporting bugs.
